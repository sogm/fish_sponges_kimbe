...
FISHES
##1. LOADING DATA FOR ALL SPECIES EXPERIMENTAL APPROACH`
```{r}
...
EAbundance_scale = scale(sp_ex_finalsum$total) 
ERichness = sp_ex_finalsum$Richness# column of Richness 96 points
ERichness_scale = scale(sp_ex_finalsum$Richness)
...
Ebiototal_env = data.frame (EAbundance, EAbundance_scale, ERichness, ERichness_scale, ETime, EGtime, ESampling,  ETreatment, EQuadrat, EQuadrat_code, EDepth, ESponge_cover, ERugosity, ECoral_cover,  EOther_cover)
```
...
```{r}
Esp_fish1_inter<-glmmTMB(EAbundance_scale~ETreatment*ETime + ERugosity + ECoral_cover+ EDepth + (1|EQuadrat/ETime), data=Ebiototal_env,
                  family=gaussian,  REML=TRUE)#nbinom2
```
...
```{r}
View(Esp_fish1.data_inter)
```

```{r, GB-fig3}
f5 <-ggplot(Esp_fish1.data_inter, aes(y=emmean, x=ETime, color=ETreatment))+
    geom_pointrange(aes(ymin = lower.CL, ymax= upper.CL), position=position_dodge(width=0.9), size=1)+
    geom_errorbar(aes(ymin= lower.CL, ymax= upper.CL), position=position_dodge(width=0.9), size=0.1, width=0.1)+
...
f5 + geom_point(data=Ebiototal_env, aes(x = ETime, y=EAbundance_scale, fill=ETreatment, alpha= 0.5), size=3,   shape = 21, col="black", position = position_jitterdodge(jitter.width = 0.5, jitter.height=0.4, dodge.width=0.9))
```
...
```{r}
Esp_fish1f.data1 = emmeans(Esp_fish1_inter, ~ERugosity, at=Esp_fish1f_grid1, type='emmean') %>%
    as.data.frame
```

```{r}
f5B<-ggplot(Esp_fish1f.data1, aes(x=ERugosity)) +
  geom_line(data= Esp_fish1f.data1 , aes(y=emmean), size=1, color="black", fill="black") +
  geom_ribbon(data=Esp_fish1f.data1,  aes(ymin=lower.CL, ymax=upper.CL), color="#35978f", fill="#35978f", alpha=0.6)+
  scale_y_continuous(breaks=seq(-6,6,2), limits=c(-5,5))+
...
f5B+geom_point(data=Ebiototal_env, aes(x = ERugosity, y=EAbundance_scale, fill= ETreatment), size=3, alpha=0.6, shape = 21, col="black")+
...
```{r}
Esp_fish1f.data1 = emmeans(Esp_fish1_inter, ~ ECoral_cover, at=Esp_fish1f_grid1, type='emmean') %>%
    as.data.frame
```
...
f5C <-ggplot(Esp_fish1f.data1, aes(x = ECoral_cover)) +
  ...
  theme(legend.position =c(), 
        ...
        legend.key = element_rect(fill = "white", color = NA))+
        theme(text=element_text(family="Times", face="bold", size=12))
f5C+geom_point(data=Ebiototal_env, aes(x = ECoral_cover, y=EAbundance_scale, fill= ECoral_cover), size=3,   shape = 21, col="black")+
...
```{r}
Esp_fish1f.data1 = emmeans(Esp_fish1_inter, ~ EDepth, at=Esp_fish1f_grid1, type='emmean') %>%
    as.data.frame
```
...
f5D <-ggplot(Esp_fish1f.data1, aes(x = EDepth)) +
  ...
  theme(legend.position =c(), 
        ...
        legend.key = element_rect(fill = "white", color = NA))+
        theme(text=element_text(family="Times", face="bold", size=12))
f5D+geom_point(data=Ebiototal_env, aes(x = EDepth, y=EAbundance_scale, fill= EDepth), size=3,   shape = 21, col="black")+
  scale_fill_distiller(trans = "reverse")
```

# 3. INTERACTION MODEL SPECIES RICHNESS EXPERIMENTAL REMOVAL DATA SET
```{r}
Espri_fish2 <- glmmTMB(ERichness_scale~ETreatment*ETime + ERugosity + ECoral_cover+ EDepth + (1|EQuadrat/ETime), data=Ebiototal_env,
...
```{r, GB-fig3}
F5E<-ggplot(Espri_fish2.data,  aes(y=emmean, x=ETime, color=ETreatment))+
    geom_pointrange(aes(ymin=lower.CL, ymax=upper.CL), position=position_dodge(width=0.9), size=1)+
    geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), position=position_dodge(width=0.9), size=0.1, width=0.1)+
  scale_y_continuous(breaks=seq(-6,6,2), limits=c(-5,5))+
  ...
  theme(legend.position =c("right"), 
        ...
        legend.key = element_rect(fill = "white", color = NA))+
        theme(text=element_text(family="Times", face=1, size=12))
F5E+ geom_point(data=Ebiototal_env, aes(x = ETime, y=ERichness_scale, fill=ETreatment, alpha= 0.5), size=3,   shape = 21, col="black", position = position_jitterdodge(jitter.width = 0.5, jitter.height=0.4, dodge.width=0.9))
```
...
f5F<-ggplot(Espri_fish2.data1, aes(x=ERugosity)) +
  geom_line(data = Espri_fish2.data1 , aes(y=emmean), size=1, color="black", fill="black") +
  geom_ribbon(data = Espri_fish2.data1,  aes(ymin=lower.CL, ymax=upper.CL), color="#35978f", fill="#35978f", alpha=0.6)+
  scale_y_continuous(breaks=seq(-6,6,2), limits=c(-5,5))+
  scale_x_continuous(breaks=seq(0,1,0.1), limits=c(0.4,0.75))+
  ylab("Species richness (Mean ± 95% CI)\n")+
  xlab("\nSubstratum complexity")+
  theme(legend.position =c(), 
        ...
        legend.key = element_rect(fill = "white", color = NA))+
        theme(text=element_text(family="Times", face="bold", size=12))
f5F+geom_point(data=Ebiototal_env, aes(x = ERugosity, y=ERichness_scale, fill= ETreatment), size=3, alpha=0.6, shape = 21, col="black")+
  scale_color_manual(values = c("black","hotpink4")) +
  scale_fill_manual(values = c("black","hotpink4"))+
  labs(fill="Sponge cover", colour="Sponge cover")#+
```
...
```{r}
Espri_fish2.data1 = emmeans(Espri_fish2 , ~ ECoral_cover, at = Espri_fish2_grid, type='emmean') %>%
    as.data.frame
```
...
f5G <-ggplot(Espri_fish2.data1, aes(x = ECoral_cover)) +
  ...
  theme(legend.position =c(), 
        ...
        legend.key = element_rect(fill = "white", color = NA))+
        theme(text=element_text(family="Times", face="bold", size=12))
f5G+geom_point(data=Ebiototal_env, aes(x = ECoral_cover, y=ERichness_scale, fill= ECoral_cover), size=3,   shape = 21, col="black")+
  scale_fill_distiller(palette = "YlOrBr", trans = "reverse")
```
...
```{r}
Espri_fish2.data1 = emmeans(Espri_fish2, ~ EDepth, at=Espri_fish2_grid, type='emmean') %>%
    as.data.frame
```
...
f5H <-ggplot(Espri_fish2.data1, aes(x = EDepth)) +
  ...
  theme(legend.position =c(), 
        ...
        legend.key = element_rect(fill = "white", color = NA))+
        theme(text=element_text(family="Times", face="bold", size=12))
f5H+geom_point(data=Ebiototal_env, aes(x = EDepth, y=ERichness_scale, fill= EDepth), size=3,   shape = 21, col="black")+
  scale_fill_distiller(trans = "reverse")
```
